...
class CapnpMessage {
  ...
  static StreamSubscription streamListener(
      Stream<Uint8List> read, Function(CapnpMessage) callback) {
    List<int> bytes = [];
    ...
    return read.listen((event) {
      if (bytes.length == 0) {
        ...
        var buffer =
            event.buffer.asUint32List(0, event.buffer.lengthInBytes ~/ 4);
  ...
  T initRoot<T>(StructBuilderFactory<T> factory) {
    ...
    StructPointer.save(segments.first.fullView(), 0, 0,
        result.layout.dataSectionLengthInWords, result.layout.numPointers);
  ...
  T newStructSegment<T>(
      SegmentView view, int offsetIntoView, StructBuilderFactory<T> factory) {
    var built = factory();
    Segment seg = Segment(this, ByteData(8 + built.layout.bytes()));
    StructPointer.save(seg.fullView(), 0, 0,
        built.layout.dataSectionLengthInWords, built.layout.numPointers);
    int segmentID = addSegment(seg);
    InterSegmentPointer.save(
        view, offsetIntoView, InterSegmentPointerType.Simple, 0, segmentID);
    return built.builder(seg.view(1, built.layout.words()));
  }

  CompositeList<T> newCompositeListSegment<T>(SegmentView view,
  ...
  void printDebug() {
    ...
    for (int i = 0; i < segments.length; i++) {
      ...
      for (int w = 0; w < segments[i].lengthInBytes ~/ 8; w++) {
        print(
            hex.encoder.convert(segments[i].data.buffer.asUint8List(w * 8, 8)));
      }
      print("");
    }
  }
  ...
  Uint8List serialize() {
    ...
    builder.add(Uint8List(4)
      ..buffer.asUint32List()[0] = segments.length - 1); // number of segments
    for (Segment seg in segments) {
      builder.add(Uint8List(4)
        ..buffer.asUint32List()[0] =
            seg.lengthInBytes ~/ 8); // number of segments
    }
